...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument("--crawl-dir", default=str(Path(__file__).resolve().parents[5] / "crawl"))
    ...
    def handle(self, *args, **opts):
        ...
        if not src.exists():
            raise CommandError(f"Không thấy {src} — chạy crawl/audio/gen_tts.py --jobs jobs_skills.jsonl map trước.")
        rows = read_csv(src)
        ...
        with transaction.atomic():
            ...
            for s in m.ShadowingSentence.objects.select_related("deck"):
                apply(s, *by_ref.get(("speaking", f"{s.deck.level_id}-{s.deck.order:02d}#{s.order}"), ("", "")), upd)
            m.ShadowingSentence.objects.bulk_update(upd, ["audio_us_path", "audio_uk_path"], batch_size=500)
            n["nói"] = (len(upd), m.ShadowingSentence.objects.count())

            upd = []
            for it in m.ListeningItem.objects.select_related("topic"):
                apply(it, *by_ref.get(("listening", f"{it.topic.level_id}-{it.topic.order:02d}#{it.order}"), ("", "")), upd)
            m.ListeningItem.objects.bulk_update(upd, ["audio_us_path", "audio_uk_path"], batch_size=500)
            n["nghe"] = (len(upd), m.ListeningItem.objects.count())

            upd = []
            for e in m.GrammarExample.objects.select_related("grammar_point"):
                apply(e, *by_ref.get(("grammar_example", f"{e.grammar_point.source_ref}#{e.order}"), ("", "")), upd)
            m.GrammarExample.objects.bulk_update(upd, ["audio_us_path", "audio_uk_path"], batch_size=500)
            ...
            for v in m.Vocabulary.objects.all():
                ...
                if hit:
                    ...
                    dangling += 1
            m.Vocabulary.objects.bulk_update(upd, ["audio_us_path", "audio_uk_path"], batch_size=500)
            ...
            for root in m.WordRoot.objects.all():
                ...
                for s in root.samples:
                    ...
                    if (us or uk) and (s.get("audio_us_path") != us or s.get("audio_uk_path") != uk):
            ...
            upd = []
            drill = {r["lesson"].split(":", 1)[1].strip("/"): by_ref[("ipa_drill", r["ref"])] for r in rows if r["target"] == "ipa_drill"}
        ...
        if dangling:
            self.stdout.write(self.style.WARNING(f"  từ vựng lộ trình có audio_path cũ nhưng không có trong bộ mới: {dangling} (kiểm tra CDN)"))
        self.stdout.write(self.style.SUCCESS("(dry-run, đã rollback)" if opts["dry_run"] else "Đã ghi."))
