...
def product():
    return Product.objects.create(
        ...
        trial_days=0,
        badge_vi="TIẾT KIỆM 50%",
        features=["Mở khoá A2–C2", "Gia sư AI"],
        order=1,
    )
...
def _event(user, type_, product="premium_year", days=365, **extra):
    ev = {
        "id": extra.pop("id", f"{type_}-{product}"),
        "type": type_,
        "app_user_id": str(user.id),
        "product_id": product,
        "original_transaction_id": extra.pop("txn", "GPA.1"),
        "store": "PLAY_STORE",
    }
    if days is not None:
...
def test_products(api, token, product):
    ...
    assert body[0]["code"] == "premium_year" and body[0]["features"][0] == {
        "title": "Mở khoá A2–C2",
        "description": "",
        "icon_url": None,
    }
...
def test_webhook_tron_doi_khong_co_han(client, settings, user):
    Product.objects.create(
        code="premium_lifetime", name_vi="Trọn đời", period="lifetime", price=999000
    )
    settings.REVENUECAT_WEBHOOK_SECRET = "topsecret"
    r = _rc(
        client,
...
def test_webhook_san_pham_la_khong_grant(client, settings, user):
    settings.REVENUECAT_WEBHOOK_SECRET = "topsecret"
    assert (
        _rc(client, "topsecret", _event(user, "INITIAL_PURCHASE", product="nope")).status_code
        == 200
    )
    user.profile.refresh_from_db()
    assert user.profile.is_premium is False
...
def test_webhook_premium_plus_keo_ca_hai_moc(client, settings, user):
    Product.objects.create(
        code="plus_month", name_vi="Premium+ Tháng", period="month", price=149000, tier="plus"
    )
    settings.REVENUECAT_WEBHOOK_SECRET = "topsecret"
...
def _subscriber(product="sayfully_premium_year", days=365, ent="premium", **sub_extra):
    exp = (
        None
        if days is None
        else (djtz.now() + timedelta(days=days)).isoformat().replace("+00:00", "Z")
    )
    return {
        "entitlements": {
            ent: {
...
def test_sync_entitlement_con_han_bat_premium(api, token, user, rc_product, monkeypatch):
    ...
    assert (
        body["is_premium"] is True and body["store"] == "play_store" and body["will_renew"] is True
    )
    sub = Subscription.objects.get(user=user)
...
def test_sync_tron_doi(api, token, user, monkeypatch):
    Product.objects.create(
        ...
        period="lifetime",
        price=999000,
        store_ids={"revenuecat": "sayfully_premium_lifetime"},
    )
    body = _sync(
        api, token, monkeypatch, _subscriber("sayfully_premium_lifetime", days=None)
    ).json()
    assert (
        body["is_premium"] is True and body["premium_until"] is None and body["will_renew"] is False
    )
...
def test_sync_tat_gia_han_va_loi_thanh_toan(api, token, user, rc_product, monkeypatch):
    body = _sync(
        ...
        monkeypatch,
        _subscriber(
            unsubscribe_detected_at="2026-09-10T00:00:00Z",
            billing_issues_detected_at="2026-09-11T00:00:00Z",
        ),
    ).json()
